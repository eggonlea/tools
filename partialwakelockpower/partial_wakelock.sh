...
TEMP=`mktemp`
BASE=`basename ${TEMP}`
DIR="/sys/fs/cgroup/${BASE}"
mkdir -p ${DIR}
echo ${ADBD} > ${DIR}/cgroup.procs
...
if [ $POWER_STAT -eq 0 ]; then
  lshal debug android.hardware.power.stats@1.0::IPowerStats/default > $OUT/power1.log
else
  dumpsys android.hardware.power.stats.IPowerStats/default delta > $OUT/power1.log
fi

$( $WORKLOAD $DURATION )

if [ $POWER_STAT -eq 0 ]; then
  lshal debug android.hardware.power.stats@1.0::IPowerStats/default > $OUT/power2.log
else
  dumpsys android.hardware.power.stats.IPowerStats/default delta > $OUT/power2.log
fi

# Log end
log -p f -t TEST "Partial Wakelock Test End"
logcat -d > $OUT/logcat.log
